...
const UploadMedia: React.FC<{ route: any; navigation: any }> = ({ 
  route, 
  navigation 
}) => {
  const { businessId, id, serviceId } = route.params;
  const [selectedImages, setSelectedImages] = useState<string[]>([]);
  const { uploadMultipleFiles, uploading, error } = useSupabaseUpload('application');
  const { insertMediaRecord, MediaUploading, Mediaerror } = useMedia();
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const mediaPromises = selectedImages.map(url => 
        insertMediaRecord(url, 'image', {
      ...
      navigation.navigate('ServiceSetupScreen', {
        businessId,
        id,
        serviceId,
      ...
      Alert.alert('Error', 'Failed to save media records');
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        <TouchableOpacity
          ...
          onPress={handleNext}
          disabled={selectedImages.length === 0 || uploading || MediaUploading}
        >
